...
In the Hotel Guest Application there are two Hyperties needed: Hotel Connectivity Hypertyand Room Monitor and Control Hyperty. The Hotel Administration part is included in the Hotel Guest Application and activated if the Application was started with the appropriate token, provides additional actions like "lock all doors", lets administrators override the hotel guest actions and has access to all the hotel rooms.
